...
sBackState_t light = PUTTOPWM;
volatile uint8_t ReadTouchWho = 1;
//初始化IIC
bool stopTim = false;
uint8_t freq = 0;
		...
		switch(I2C_GetLastEvent(I2C1)) {
			...
			case I2C_EVENT_SLAVE_BYTE_TRANSMITTED: //EV3
				...
				if(NumTime == 0){
					...
					//printf("%d\r\n",(MyTouchData.yHigh[ReadTouchWho-1]&0x0F) | (MyTouchData.finger[ReadTouchWho-1]) << 4);
					NumTime++;
				}else if( NumTime    == 2){
					I2C_SendData(I2C1, MyTouchData.yLow[ReadTouchWho-1]);
					//printf("%d\r\n",MyTouchData.yLow[ReadTouchWho-1]);
					NumTime = 0;
				}
				break;
